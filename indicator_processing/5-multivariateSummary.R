...
d <- matrix_data[which(matrix_data$year >= 2000 & matrix_data$year <= 2024), ]
...
risks <- dm$desc_name[which(dm$type == "risk")]
fi <- dm$desc_name[which(dm$type == "fi")]
ccl <- dm$desc_name[which(dm$type == "ccl")]
oth <- dm$desc_name[which(dm$type == "oth")]
...
png(filename = "indicator_plots/traffic.png", units="in", width=18, height=14, pointsize=24, res=72*12)

nf <- layout(matrix(c(1:5, 5), 3, 2, byrow = TRUE), widths = c(6, 6), heights = c(4.5, 4.5, 0.8), respect = TRUE)
#layout.show(nf)

par(mar = c(2, 11, 0.5, 0.5))

trafficLightPlot(d_risk, noNAs = 1, cexlabs = 0.8, splits = 5)
mtext(side = 2, "Risks to meeting objectives", line = 9.5, cex = 0.9)
trafficLightPlot(d_fi, noNAs = 1, cexlabs = 0.8)
mtext(side = 2, "Fishery-independent indicators", line = 9, cex = 0.9)
trafficLightPlot(d_oth, noNAs = 1, cexlabs = 0.8)
mtext(side = 2, "Other management indicators", line = 9.5, cex = 0.9)
trafficLightPlot(d_ccl, noNAs = 1, cexlabs = 0.8)
mtext(side = 2, "Fishery-dependent indicators", line = 9, cex = 0.9)

plot.new()
par(mar = c(0, 0, 0, 0)+0.2)
legend("bottom", c("0-20%    ", "20-40%    ", "40-60%    ", "60-80%    ", "80-100%    "), 
       col = c("#FF000099", "#FF900089", "#CCFF0079", "#0090FF89", "#0000FF99"), 
